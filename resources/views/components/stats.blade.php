<section class="container dati" id="stats">
    <h1 class=" titolo_numeri text-center" >{{__('ui.numbers')}}
    </h1>
    
    <div class="row justify-content-around flex-row">
        
        <div class="col-md-3 dato pt-3 text-center" data-aos="zoom-in" data-aos-duration="900">
            <i class="bi bi-award-fill stats-item"></i>
            <h4 class="text-center text-bianco fs-5">{{__('ui.years')}}</h4>
            <p class="text-center text-nero stats-number"><span id="firstNumber" class="number">0</span></p>
        </div>
        
        <div class="col-md-3 dato pt-3 text-center" data-aos="zoom-in"data-aos-duration="1900">
            <i class="bi bi-people-fill stats-item"></i>
            <h4 class="text-center text-bianco fs-5">{{__('ui.skill')}}</h4>
            <p class="text-center text-nero stats-number"><span id="secondNumber" class="number">0</span></p>
        </div>
        
        <div class="col-md-3 dato pt-3 text-center" data-aos="zoom-in"data-aos-duration="2500">
            <i class="bi bi-box-seam-fill stats-item"></i>
            <h4 class="text-center text-bianco fs-5">{{__('ui.totale')}}</h4>
            <p class="text-center text-nero stats-number"><span id="thirdNumber" class="number">0</span></p>    
        </div>
        
        <div class="col-md-3 dato pt-3 text-center" data-aos="zoom-in" data-aos-duration="3000">
            <i class="bi bi-cart3 stats-item"></i>
            <h4 class="text-center text-bianco fs-5">{{__('ui.order')}}</h4>
            <p class="text-center text-nero stats-number"><span id="fourthNumber" class="number">0</span></p>
        </div>
        
    </div>
    
</section>